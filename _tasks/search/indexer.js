var indexer = require("markdown2json").Indexer; //markdown2json
var fs = require("fs");
		...
		for(var key in item){
			if(item[key] instanceof Array){
				...
				for(var k in item[key]){
					for(var y in item[key][k]){
						newItem[key] += cleanMdContent(item[key][k][y]);
					}
				}
			}else{
...
function saveAzure(idx){
	...
	idx = updateSchema(idx, azureIndexName);
